...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        var
        for var in get_Config().model_config.features
        if var
        not in get_Config().model_config.categorical_vars_with_na_frequent
        + get_Config().model_config.categorical_vars_with_na_missing
        + get_Config().model_config.numerical_vars_with_na
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    ...
    input_data.rename(columns=get_Config().model_config.variables_to_rename, inplace=True)
    input_data["MSSubClass"] = input_data["MSSubClass"].astype("O")
    relevant_data = input_data[get_Config().model_config.features].copy()
